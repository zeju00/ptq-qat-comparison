...
class ComplexCNN(nn.Module):
    def __init__(self):
        ...
        # Quantization Stubs (only for QAT)
    ...
    def fuse_model(self):
        ...
        self.eval()  # Switch to eval mode for fusion
        fuse_modules(self, [
            ['conv1', 'bn1', 'relu1'],
            ['conv2', 'bn2', 'relu2'],
            ['conv3', 'bn3', 'relu3'],
...
ptq_model.eval()
with torch.no_grad():
    for data, _ in tqdm(test_loader, desc='PTQ Calibration', unit='batch'):
        ptq_model(data)
end_calibration_time = time.time()
calibration_time = end_calibration_time - start_calibration_time
...
# Use per-tensor quantization scheme to avoid the issue
'''
...
start_fine_tuning_time = time.time()
for i in range(1):
    print(f'Epoch {i+1}:')
    for data, target in tqdm(train_loader, desc="QAT Fine-tuning", unit="batch"):
        ...
        loss.backward()
        optimizer.step()
end_fine_tuning_time = time.time()
qat_fine_tuning_time = end_fine_tuning_time - start_fine_tuning_time
print(f'QAT Fine-Tuning time: {qat_fine_tuning_time} seconds')
...
df['QAT_Accuracy_Min'] = df['QAT_Accuracy'].min()
